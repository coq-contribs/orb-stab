...
COQLIBS?= -R ../../Nijmegen/LinAlg LinAlg\
  -R ../../Sophia-Antipolis/Algebra Algebra\
  -R . OrbStab
COQDOCLIBS?=-R ../../Nijmegen/LinAlg LinAlg\
  -R ../../Sophia-Antipolis/Algebra Algebra\
  -R . OrbStab
...
uninstall_me.sh:
	echo '#!/bin/sh' > $@ 
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/OrbStab && rm -f $(VOFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "OrbStab" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
